...
describe('Section 1: Functional tests', () => {
    ...
    it('User can use only same both first and validation passwords', ()=>{
        ...
        cy.get('#username').type('AgneKaruse');
        cy.get('#email').type('agnekaruse@gmail.com');
        cy.get('[data-cy="name"]').type('Agne');
        cy.get('#lastName').type('Karuse');
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777');
        cy.get('#password').type('Paikesekiir');

        // Type confirmation password which is different from first password
        cy.get('#confirm').type('rebane');

        //Click somewhere otside the password field to activate sumbit button
        ...
        // Assert that error message is visible and dipalyes message "Password do not match!"
        cy.get('#password_error_message').should('be.visible').should('contain', 'Passwords do not match!');

        //Change the test, so the password will match
        cy.get('#confirm').clear();
        cy.get('#confirm').type('Paikesekiir');
        cy.get('h2').contains('Password').click();
    ...
    it('User can submit form with all fields added', ()=>{
        ...
        cy.get('#success_message').should('be.visible', ' User successfully submitted registration');
    })
    ...
    it('Phone number field is absent', () => {
        cy.get('#username').type('JohnDoe');
        cy.get('#email').type('validemail@yeap.com');
        cy.get('[data-cy="name"]').type('John');
        cy.get('#lastName').type('Doe');
